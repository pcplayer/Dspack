...
interface
uses
  BaseClass, Winapi.DirectShow9, DSUtil,
...
type
  ...
  TBCPushPinBitmap = class(TBCSourceStream{,IPushBMP})
  protected
    ...
    //add by pcplayer
    //FBmpIntf:ITestComReadMainThread;
    FFileBuffer_2: PByte;
    FCounter:Integer;

    //add by pcplayer uses mapped file
    //FMapFile:TMapStream;
  public
   ...
   // wait until we're unblocked. No frames are ever dropped.
    ...
    function Notify(Filter: IBaseFilter; q: TQuality): HRESULT; override;
      stdcall;
    procedure PushBMP(BMP:PChar;ASize:Integer);safecall; //add by pcplayer
  end;
...
var
  ...
  bmpData:WideString; //add by pcplayer
begin
  inherited Create('_ Push Source Bitmap', hr, Filter, 'Out');

  //CoInitialize(nil);
  //FBmpIntf:=CoTestComReadMainThread.Create; //add by pcplayer

  {///开始用 mapped file
  try
    FMapFile:=TMapStream.Create('C:\temp.tmp');
    FMapFile.FileOptions:=foOpenExisting;

    FMapFile.Active:=True;
    FMapFile.Size:=900000;
  except
    raise Exception.Create('生成mapped file失败!');
  end;
   }
  /// mapped file end
...
{$ENDIF}
  ...
  FFileBuffer_2:= CoTaskMemAlloc(FileSize);
  if (FFileBuffer_2 = nil) then
  begin
    OutputDebugString('Could not allocate FImage 2');
    hr := E_OUTOFMEMORY;
    Exit;
  end;
  ...
  //add by pcplayer
  {
  if Assigned(FBmpIntf) then  //add by pcplayer
    begin
      FBmpIntf.GetBMP(bmpData);
      Move(bmpData[1],FFileBuffer_2^,Length(bmpData));
          // Store a pointer to the BITMAPINFO
      pb := PByte(FFileBuffer_2);
      Inc(pb, FileHeaderSize);
      FBmi := PBITMAPINFO(pb);
...
{$ENDIF}
  ...
  if Assigned(FFileBuffer_2) then
  begin
    CoTaskMemFree(FFileBuffer_2);
    FFileBuffer_2 := nil;
  end;
  ...
  /// add by pcplayer free mapped file
  //if Assigned(FMapFile) then FMapFile.Free;
...
var
  ...
  bmpData:String; //add by pcplayer
  DataSize:LongWord; //add by pcplayer
...
//        FBmpIntf:=nil;
//      end;
//      Move(bmpData[1],FImage^,Length(bmpData));
//
//      pb := PByte(FFileBuffer_2);
//      FileHeaderSize := SizeOf(BITMAPFILEHEADER);
//      Inc(pb, FileHeaderSize);
...
//    end;
  ...
  ///add by pcplayer use mapped file below

  {
  try
    SetLength(bmpData,FMapFile.Size);
    FMapFile.Position:=0;
    FMapFile.Read(bmpData[1],FMapFile.Size);
  except
    raise Exception.Create('把mapped file内容读到字符串缓冲里面去失败');
  end;

  try
    FMapFile.Position:=0;
    FMapFile.Read(DataSize,SizeOf(LongWord));
  except
    raise Exception.Create('取得图片大小尺寸数据失败');
  end;

  if DataSize>0 then
  begin
    try
      Delete(bmpData,1,SizeOf(LongWord));

      Move(bmpData[1],FImage^,DataSize);
    except
      raise Exception.Create('把图片数据从字符串缓冲搬进FImage失败');
    end;
  ...
  try
    ...
    //Move(FImage^,pData^,min(pvi.bmiHeader.biSizeImage, cbData));
    CopyMemory(pData,FImage, min(pvi.bmiHeader.biSizeImage, cbData));
    Inc(FCounter);
   // CopyMemory(pData,FImage, pvi.bmiHeader.biSizeImage);
...
begin
  ...
  try
    ...
    begin
      FBmpIntf.GetBMP(bmpData);
      Move(bmpData[1],(FFileBuffer[2])^,Length(bmpData));

      pb := PByte(FFileBuffer[2]);
      FileHeaderSize := SizeOf(BITMAPFILEHEADER);
      Inc(pb, FileHeaderSize);
      FBmi[2] := PBITMAPINFO(pb);

      // Store a pointer to the starting address of the pixel bits
      Inc(pb, FBitmapInfo[2]);

      AImage := pb;

      CopyMemory(pData, AImage, min(pvi.bmiHeader.biSizeImage, cbData));
    end;
     }
...
procedure TBCPushPinBitmap.PushBMP(BMP: PChar; ASize: Integer);
begin
{---------------------------------------------------------
  add by pcplayer 向BMP Filter输入图片
-----------------------------------------------------------------------------}
  FImage:=PByte(BMP);
end;
