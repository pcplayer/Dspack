...
type
  TFormPlayWin = class(TForm)
    ...
    procedure OSD1Click(Sender: TObject);
  private
...
procedure TFormPlayWin.OSD1Click(Sender: TObject);
begin
...
begin
  ...
  //WindowlessÄ£Ê½ÏÂ£º
  if btFullScreen.Down then
  begin
    VideoWindow.VMR9.SetVideoClippingWindow(Self.Handle);
  end
  else
  begin
    VideoWindow.VMR9.SetVideoClippingWindow(FmFullScren.Handle);
    FmFullScren.Show;
  end;



  btFullScreen.Down := not btFullScreen.Down; // VideoWindow.FullScreen;
end;
